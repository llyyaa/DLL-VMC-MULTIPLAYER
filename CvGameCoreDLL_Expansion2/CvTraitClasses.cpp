...
CvTraitEntry::CvTraitEntry() :
	...
	m_bScienceOutputFromArtistConsumption(false),
...
bool CvTraitEntry::IsScienceOutputFromArtistConsumption() const
{
	return m_bScienceOutputFromArtistConsumption;
}
...
bool CvTraitEntry::CacheResults(Database::Results& kResults, CvDatabaseUtility& kUtility)
{
	...
	m_bScienceOutputFromArtistConsumption = kResults.GetBool("ScienceOutputFromArtistConsumption");
...
void CvPlayerTraits::InitPlayerTraits()
{
	for(int iI = 0; iI < GC.getNumTraitInfos(); iI++)
	{
		if(HasTrait((TraitTypes)iI))
		{
			...
			if(trait->IsScienceOutputFromArtistConsumption())
			{
				m_bScienceOutputFromArtistConsumption = true;
			}
...
void CvPlayerTraits::Read(FDataStream& kStream)
{
	...
	kStream >> m_bScienceOutputFromArtistConsumption;
